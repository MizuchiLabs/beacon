<script lang="ts">
	...
	const chartData = $derived(
		points.map((p) => {
			...
			return {
				...
				lineDate: new Date(new Date(p.timestamp).getTime() + step / 2),
	...
	const lineSeries = $derived(
		chartData.some((d) => d.avg_norm != null)
			? [{ key: 'avg_norm', label: 'Response time', color: 'var(--primary)' }]
			: []
	);
		...
		{#if lineSeries.length > 0}
			<div class="pointer-events-none absolute inset-0">
				<LineChart
					...
					series={lineSeries}
					props={{
						line: { strokeWidth: 1.5, motion: 'tween' }
					}}
					padding={{ top: 0, bottom: 0, left: 0, right: 0 }}
				/>
			</div>
