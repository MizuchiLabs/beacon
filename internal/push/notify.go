...
func NewNotifier(ctx context.Context, conn *db.Connection) *Notifier {
...
func (n *Notifier) SendMonitorUpNotification(ctx context.Context, monitor *db.Monitor) error {
	...
	for _, sub := range subscriptions {
		if err := n.sendPushNotification(sub, payloadBytes); err != nil {
			...
			if isSubscriptionError(err) {
				n.conn.Queries.DeletePushSubscriptionByEndpoint(ctx, sub.Endpoint)
			}
		}
	}
...
func (n *Notifier) sendPushNotification(
	...
	resp, err := webpush.SendNotification(payload, sub, &webpush.Options{
		Subscriber:      "mailto:your-email@example.com", // Update with your email
